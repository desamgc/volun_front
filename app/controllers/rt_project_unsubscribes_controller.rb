class RtProjectUnsubscribesController < ApplicationController
  ...
  def create
    @rt_project_unsubscribe = RtProjectUnsubscribe.new(rt_project_unsubscribe_params)
    @rt_project_unsubscribe.build_request_form
    @rt_project_unsubscribe.request_form.user_id = current_user.id
    if @rt_project_unsubscribe.save
      redirect_to projects_url, notice: t('rt_project_unsubscribe.response')
    else
